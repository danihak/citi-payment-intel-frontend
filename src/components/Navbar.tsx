...
export function Navbar({ currentPage, onNavigate }: NavbarProps) {
  ...
  const scoreColor = healthScore === 100 ? '#34D399' : healthScore >= 80 ? '#FCD34D' : '#F87171'
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 24px', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
              <rect x="1" y="1" width="6" height="6" rx="1" fill="#38BDF8" opacity="0.9"/>
              <rect x="11" y="1" width="6" height="6" rx="1" fill="#38BDF8" opacity="0.5"/>
              <rect x="1" y="11" width="6" height="6" rx="1" fill="#38BDF8" opacity="0.5"/>
              <rect x="11" y="11" width="6" height="6" rx="1" fill="#38BDF8" opacity="0.9"/>
            </svg>
            <span style={{ fontWeight: 600, fontSize: 13, color: '#F9FAFB' }}>India Payment Intelligence Hub</span>
            <span style={{ color: '#374151', fontSize: 11 }}>| CITI TTS</span>
          </div>
          <div style={{ display: 'flex', gap: 2 }}>
            {(['dashboard', 'incidents', 'compliance'] as const).map(page => (
              <button key={page} onClick={() => onNavigate(page)} style={{
                ...
                background: currentPage === page ? 'rgba(56,189,248,0.15)' : 'transparent',
                color: currentPage === page ? '#38BDF8' : '#9CA3AF',
                borderBottom: currentPage === page ? '2px solid #38BDF8' : '2px solid transparent',
              }}>
                {page}
                {page === 'incidents' && activeIncidents > 0 && (
                  <span style={{ marginLeft: 6, background: '#F87171', color: '#fff', borderRadius: 10, padding: '1px 5px', fontSize: 9, fontFamily: 'IBM Plex Mono' }}>{activeIncidents}</span>
                )}
              </button>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ color: '#4B5563', fontSize: 10, fontFamily: 'IBM Plex Mono', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Health</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: allCompliant ? '#34D399' : '#F87171', display: 'inline-block' }} />
            <span style={{ fontSize: 11, color: '#9CA3AF', fontFamily: 'IBM Plex Mono' }}>OC-215</span>
          </div>
          <div style={{ width: 1, height: 16, background: 'rgba(255,255,255,0.08)' }} />
          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#34D399', display: 'inline-block' }} />
            <span style={{ fontSize: 11, color: '#9CA3AF', fontFamily: 'IBM Plex Mono' }}>LIVE</span>
          </div>
        </div>
      </div>
    </nav>
